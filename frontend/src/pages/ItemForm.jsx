...
export default function ItemForm() {
  ...
  async function handleSubmit(e) {
    ...
    const validPrices = prices.filter((p) => p.price !== '' && p.price !== null)
    ...
    if (isEdit) {
      await supabase
        ...
        .eq('id', id)
      // Cara sederhana & aman: hapus semua harga lama, insert ulang (riwayat tetap tercatat via trigger)
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5 pb-8">
      <h1 className="font-display text-2xl font-bold">{isEdit ? 'Edit Barang' : 'Tambah Barang'}</h1>
      ...
      <Field label="Lokasi">
        ...
        <div className="flex gap-2 mt-2">
          ...
          />
          <button type="button" onClick={addLocation} className="px-3 rounded-xl border border-line dark:border-[#2C332A] text-sm">
            Tambah
          </button>
        </div>
      </Field>
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            ...
            className="text-xs text-forest dark:text-mustard font-medium"
          >
            + Tambah satuan harga
          </button>
        </div>
        <div className="flex flex-col gap-2">
          {prices.map((row, idx) => (
            <div key={idx} className="grid grid-cols-[1fr_70px_1fr_auto] gap-2 items-center rounded-xl border border-line dark:border-[#2C332A] p-2.5">
              <input
      ...
      <style>{`
        ...
        .dark .input { background: #1D231C; border-color: #2C332A; }
        .input:focus { border-color: var(--color-forest); }
      `}</style>
    </form>
  )
}
